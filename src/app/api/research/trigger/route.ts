...
async function triggerApifyResearch(
  ...
  const websiteTask = async () => {
    try {
      ...
      // This crawls up to 300 pages using sitemap URLs + homepage as seeds
      ...
      if (crawlResult.success && crawlResult.pages.length > 0) {
        const enriched = enrichCrawlResult(crawlResult.pages, input.website, input.businessName, 300);
